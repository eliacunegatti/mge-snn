...
def random_growth(masking, name, new_mask, total_regrowth, weight):
    if masking.prune_mode == 'SET':
        n = (new_mask==0).sum().item()
        total_regrowth = masking.name2nonzeros[name] - (new_mask==1).sum().item()
        if n == 0: return new_mask
        while(True):
            expeced_growth_probability = (total_regrowth/n)
            new_weights = torch.rand(new_mask.shape).cuda() < expeced_growth_probability

            k = new_mask.bool() |  new_weights
            if k.sum().item() ==  new_mask.sum().item() + total_regrowth:
                break
    else:
        n = (new_mask==0).sum().item()
        if n == 0: return new_mask
