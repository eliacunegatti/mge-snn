...
def add_sparse_args_SNFS(parser):
    ...
    parser.add_argument('--sparse-init', type=str, default='ERK', help='Initialization of weights in layer.', choices=['uniform', 'ER', 'ERK', 'ERK_plus', 'resume'])
...
class Masking(object):
    ...
    def __init__(self):
        ...
        #self.fc_density = None


    def init(self, mode='uniform', density=0.05,erk_power_scale=1.0):
        ...
        if mode == 'uniform': ##or
            ...
            total_sparse_params = total_params * density

            name1 = list(self.masks.keys())[0]
            name2 = list(self.masks.keys())[-1]
            
            print(list(self.masks.keys()))

            # remove the first layer
            total_sparse_params = total_sparse_params - self.masks[name1].numel()
            mask1 = self.masks.pop(name1)
            
            print(name1, mask1.shape)

            if density < 0.2:
                total_sparse_params = total_sparse_params - self.masks[name2].numel() * 0.2
                density = float(total_sparse_params / total_params)
            ...
            name1 = list(self.masks.keys())[-1]
            total_params = total_params - self.masks[name1].numel()
            density = (self.baseline_nonzero - self.masks[name1].numel() * self.fc_density) / total_params
            mask1 = self.masks.pop(name1)
    ...
    def add_module(self, module, density, sparse_init='constant'):
        ...
        self.remove_type(nn.BatchNorm2d, verbose=True)
        #print('Removing 1D batch norms...')
        self.remove_type(nn.BatchNorm1d, verbose=True)
        #self.fc_density = density
        ...
        for module in self.modules:
            for name, tensor in module.named_parameters():
                ...
                if name in self.name2variance:
                    ...
                    print(val)
                    print('Non zeros: ', num_nonzeros, 'Zeros:', num_zeros, 'Sum', num_zeros+num_nonzeros, 'Real Total', total_params)
                else:
                    print('Stats', name, num_nonzeros, num_zeros, 'Sparsity:', num_zeros / (num_nonzeros+num_zeros), 'Density:',num_nonzeros / (num_nonzeros+num_zeros))
