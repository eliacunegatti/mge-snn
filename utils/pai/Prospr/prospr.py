...
import utils.pai.Prospr.utils as utils
...
def get_dataset(dataset):
    if dataset == 'CIFAR-10':
        ...
        channels = 3
        classes = 10
        trainloader_ = torch.utils.data.DataLoader(trainset, 512, shuffle=True)
        testloader_ = torch.utils.data.DataLoader(testset, 512, shuffle=False)
        ...
        from tiny_imagenet.tiny_imagenet_first import TinyImageNet
        batch_size = 512
        C = TinyImageNet(batch_size)
        trainloader_ = (C.train_loader)
        testloader_ = (C.val_loader)
        del C
        #valoader = C.actual_val_loader
        channels = 3
        classes = 200
        n = 32
    else:
    ...
    return trainloader_, testloader_, n, channels, classes
...
def prune(
    ...
    dataloader_ , _, _, _, _ = get_dataset(dataloader)

    outer_dataloader = iter(dataloader_)
